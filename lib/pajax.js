// import {resolvePipelets, assignOpts, mergeOpts, decorate} from './utils';
...
class Pajax {
  ...
  constructor(opts={}, init={}) {
    ...
    this.Request = init.Request || Request;
    this.Response = init.Response || Response;
    this.requestData = init.requestData;
  }
  ...
  fget(url, opts) {
    return this.request(url, opts, 'GET').fetch();
  }

  fhead(url, opts) {
    return this.request(url, opts, 'HEAD').fetch();
  }

  fpost(url, opts) {
    return this.request(url, opts, 'POST').fetch();
  }

  fput(url, opts) {
    return this.request(url, opts, 'PUT').fetch();
  }

  fpatch(url, opts) {
    return this.request(url, opts, 'PATCH').fetch();
  }

  fdelete(url, opts) {
    return this.request(url, opts, 'DELETE').fetch();
  }

  fdel(...args) {
    return this.fdelete(...args);
  }
  ...
  fetch(url, opts) {
    let req;
    if (url instanceof this.Request && !opts) {
      req = url;
    } else {
      // merge default opts into request opts
      opts = mergeOpts(this.opts, opts);
      req = new this.Request(url, opts)
    }
    ...
    .then(()=>{
      return new Promise((resolve, reject) => {
        ...
        // Callback for errors
        xhr.onerror = () => {
          var res = new this.Response(req, xhr);
