...
class Pajax {
  ...
  request(url, init) {
    ...
    let classDefaults = this.constructor.defaults;
    // Merge pajax/ResponseCtor, class defaults and instance defaults
    init = options({pajax:this,Response:ResponseCtor}, classDefaults, this.defaults, init);
    return new RequestCtor(url, init);
  }
  spawn(init) {
    init = options(this.defaults, init);
    return this.fork(init);
  }
  fork(init) {
