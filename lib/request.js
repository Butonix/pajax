...
export default class PajaxRequest {
  ...
  constructor(method, url, opts, factory) {
    if(!method || !url) {
      throw 'URL and method required for request';
    }
  ...
  clone(url, opts) {
    url = url || this.url;
    opts = _.clone(opts || this.opts);
    var req = new PajaxRequest(this.method, url, opts, this.factory);
  ...
  send() {
    ...
    .then(res=>{
      // SUCCESS: if no custom error and status between 200 and 299 or 304
      if(!res.error && ((res.status >= 200 && res.status < 300) || res.status===304)) {
        return Promise.resolve(res);
      } else {
        // When no custom error is set
        if(!res.error) {
          // Unknown status code
