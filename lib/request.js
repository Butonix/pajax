...
export default class PajaxRequest extends Options {
  ...
  constructor(url, init={}, {factory, requestData, Response}={}) {
    super(init);
  ...
  get processedURL() {
    ...
    if (this.opts.queryParams) {
      qp = _.merge(qp, this.opts.queryParams);
    }
    ...
    if(parsedURL.search) {
      ...
      if(search) {
        qp = _.merge(qs.parse(search), qp);
      }
    }

    // Merge caching param into qp hash
    if(this.method === 'GET' && this.opts.cache==='no-cache') {
      var ts = Date.now();
      qp._ = ++ts;
    }
  ...
  send() {
    ...
    return Promise.resolve().then(()=>{
      return resolvePipelets(this.opts.pipelets.before || [], this);
    })
    .then(()=>{
      return new Promise((resolve, reject) => {
        ...
        // Set withCredentials
        if(this.opts.credentials==='include') {
    ...
    .then(res=>{
      ...
      return resolvePipelets(this.opts.pipelets.after || [], res);
    }).then(res=>{
      if(res.error) {
        // Resolve afterFailure pipelets
        return resolvePipelets(this.opts.pipelets.afterFailure || [], res);
      } else {
        // Resolve afterSuccess pipelets
        return resolvePipelets(this.opts.pipelets.afterSuccess || [], res);
      }
    }).then(res=>{
