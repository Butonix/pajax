...
export default class PajaxRequest {
  ...
  createResult(xhr) {
    if(this.factory && typeof this.factory.result==='function') {
      // Use callback if available on factory
      return this.factory.result(this, xhr);
    } else {
  ...
  send(url) {
    ...
    .then(res=>{
      ...
      // Failure: if custom error or status below 200 or beyond 299 excluding 304
